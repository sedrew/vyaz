...
import type { LineBox, FragmentBox, ParagraphLayoutResult } from '../types/LayoutTypes.js';
import type { DebugFlags } from '../types/RenderTypes.js';
...
export function renderToSVG(lines: LineBox[], options: SVGRenderOptions = {}): string {
  ...
  for (const line of lines) {
    if (opts.structure === 'glyph') {
      ...
      // Each run gets its own <text> element with id="${paragraphId}-${itemIndex}"
      let currentRunIdx = -1;
      for (const frag of line.fragments) {
